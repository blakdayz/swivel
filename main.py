...
from src.swivel.scanners.ble_scanner import BLEScanner, run_scanner, create_tables
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            for device in devices:
                ...
                places = [f"{place.name} ({place.latitude}, {place.longitude})" for place in device.places]
...
async def report_multiple_places_with_gatt_check():
    ...
    try:
        async with AsyncSessionLocal() as session:
            reports = await ble_scanner.report_devices_seen_in_multiple_places_with_gatt_check()
            return json.dumps(reports)
    except Exception as e:
        logging.error(f"Error generating multiple places report with GATT signature check: {e}")
        raise HTTPException(status_code=500, detail="Internal Server Error")


if __name__ == "__main__":
    asyncio.run(start_scanning())  # Start the BLE scanner upon starting the application
